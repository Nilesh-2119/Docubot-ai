...
async def add_google_sheet(
    ...
    """Add a Google Sheet source, fetch data, and create embeddings."""
    # Validate URL
    ...
    try:
        ...
        # Store embeddings (use sheet.id as document_id for easy cleanup)
...
async def sync_sheet(db: AsyncSession, sheet: GoogleSheet) -> bool:
    """Sync a Google Sheet — re-embed only if data changed. Returns True if updated."""
    try:
        ...
        # Data changed — delete old embeddings and re-embed
        await db.execute(
...
async def remove_google_sheet(db: AsyncSession, sheet_id: str, chatbot_id: str) -> bool:
    """Delete a Google Sheet and its embeddings."""
    result = await db.execute(
